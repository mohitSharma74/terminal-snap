...
This project includes specialized skill guides for AI assistants:
- **Frontend Engineering** (`.claude/frontend-engineering.md`): Component patterns, state management, TypeScript conventions
- **UI/UX Design** (`.claude/ui-ux-design.md`): Glassmorphic design system, color palette, animations, accessibility
- **Code Review** (`.claude/code-review.md`): Review checklist, quality standards, common anti-patterns
- **Testing** (`.claude/testing.md`): Testing conventions, mock patterns, snapshot usage, coverage expectations

When working on tasks related to these areas, refer to the appropriate skill guide for detailed standards and patterns.
...
- `TerminalSettings`: Main state object combining all customization options (includes transparentBackground boolean)
...
- 10 predefined terminal themes:
  - **Dark**: Dracula (default), Nord, One Dark, Gruvbox Dark, Tokyo Night, Catppuccin Mocha, Catppuccin Frappe, Catppuccin Macchiato
...
- Catppuccin themes use official color specifications from the Catppuccin palette project

### Background System

Backgrounds are defined in `src/lib/backgrounds.ts`:
- 15+ preset options including gradients, solid colors, and macOS-inspired abstract wallpapers
- Each preset has: id, name, CSS value (gradient or color), and preview color
- Background CSS is applied directly to the preview container's style prop (or set to transparent if transparentBackground is enabled)
...
- macOS abstract wallpapers use layered radial gradients to create depth and visual interest (Sonoma Purple, Sequoia Blue, Green Aurora, Sunset Orange, Pink Flow)
...
The transparent background toggle (`TransparentBackgroundToggle.tsx`) allows users to make the outer padding area transparent while keeping the terminal window solid:
- When enabled, the preview container's background is set to `"transparent"` instead of the selected background CSS
- The terminal window itself (the inner rounded box with the terminal content) remains solid with the theme's background color
- The BackgroundSelector component is automatically disabled when transparent background is active (receives `disabled` prop)
- This feature is useful for users who want to overlay terminal screenshots on custom backgrounds in their presentations or documentation
- The transparent background state is stored in `TerminalSettings.transparentBackground` (boolean)
- Component follows the same pattern as `DropShadowToggle` using `ModernCheckbox` UI component
...
- **Left Panel (Controls)**: GlassCard containing all customization controls
  ...
  - Background selector (floating label, disabled when transparent background is enabled)
